...
// เกณฑ์การมาปฏิบัติงาน (โมเดล "เป็นธรรม" — เจ้าของนโยบายเคาะ 2026-09-14 · ตรงกับ ATT_DEFAULT_FIELDS ใน app.js).
// นับ (หักคะแนน) เฉพาะ ขาด · สาย · ลาป่วยไม่มีใบรับรอง · สิทธิ์ลาที่ชอบธรรม = deduct 0 (บันทึกได้ ไม่หัก).
// ค่านี้ใช้ตอน seed PT-A/B/C ลง DB ครั้งแรกเท่านั้น (ON CONFLICT DO NOTHING) — DB ที่ seed ไปแล้ว
// เก็บเกณฑ์เดิมไว้ (ถูกล็อก · แก้โดยคัดลอกเป็นเวอร์ชันใหม่ในตัวแก้เทมเพลตฝั่ง HR).
...
// A start_date whose year lands in the Buddhist-era range (>= 2400) was entered/imported as พ.ศ.
// — e.g. a CSV typed "2569-08-03". No real hire date is a Gregorian year >= 2400, so convert it
// back to ค.ศ. (−543) instead of storing a date that renders 543 years in the future (2569 → 3112).
function normalizeGregorianDate(str) {
  const s = clean(str);
...
function validateEmployeeInput(input, rowNumber = 0) {
  ...
  const startDate = normalizeGregorianDate(input.startDate);
...
async function getTask(request, env) {
  ...
  if (EVAL_TASK_TYPES.includes(task.task_type)) {
    ...
    // Carry the KPI structure (topics + grade criteria) forward from the FIRST round that has any
    // content — even a saved draft — so rounds 60/90 inherit round 30's items without waiting for it
    // to be submitted. ASC (not DESC) anchors every later round to round 1's content, which the UI
    // then locks read-only; scores are re-entered per round (defaultEvalState resets them).
    const prev = await env.DB.prepare(`
      ...
      ORDER BY COALESCE(month_no, 0) ASC LIMIT 1
    ...
    probation = {
      ...
      template: caseForm || (template ? {
        ...
        name: template.name,
        // level drives the "(Form A/B/C)" label on the printed doc + CSV. Without it the frontend
        // fell back to 'staff' for every template, so every form printed as "(Form B)".
...
async function requireAdmin(request, env) {
  const input = await readJson(request);
  // SECURITY: a deactivated account must lose admin access immediately — mirror the active
  // check that requireViewer/resolveActor already enforce. Without this, a deactivated HR still
  // passes (their web-session token stays valid for its TTL, and a fresh LINE idToken can always
  // be minted), letting them re-activate themselves or hit any admin route.
  if (input.webSessionToken) {
    const webUser = await getUserFromWebSession(env, input.webSessionToken);
    if (!webUser || !webUser.active || webUser.role !== "HR") throw new Error("Admin web session required.");
    return { input, user: webUser };
  }
  const verified = await resolveLineIdentity(env, input);
  const user = await getUserByLine(env, verified.sub);
  if (!user || !user.active || user.role !== "HR") throw new Error("Admin access required.");
  return { input, user };
}
...
async function adminUpdateUser(request, env) {
  ...
  // SECURITY: on deactivation, kill any live web sessions so the account can't keep acting
  // through a token issued before it was disabled (the idToken path is already blocked by the
  // active check in requireAdmin/requireViewer/resolveActor).
  if (!updated.active) {
    try { await env.DB.prepare("DELETE FROM web_sessions WHERE user_id = ?").bind(userId).run(); } catch (e) { /* web_sessions may not exist yet */ }
  }
...
// One-time repair: employees whose start_date was entered/imported as a Buddhist-era year (>= 2400)
// render 543 years in the future (2569 → 3112). Rewrite them to Gregorian (−543). HR-only and
// idempotent — a year < 2400 is never touched, so re-running is a no-op.
async function adminFixBuddhistDates(request, env) {
  const { user: actor } = await requireAdmin(request, env);
  const res = await env.DB.prepare(
    "SELECT employee_id, start_date FROM employees WHERE start_date IS NOT NULL AND start_date <> '' AND CAST(substr(start_date, 1, 4) AS INTEGER) >= 2400"
  ).all();
  const rows = res.results || [];
  const ts = nowIso();
  let fixed = 0;
  for (const r of rows) {
    const fixedDate = normalizeGregorianDate(r.start_date);
    if (fixedDate === r.start_date) continue; // not YYYY-MM-DD / already ค.ศ. — leave untouched
    await env.DB.prepare("UPDATE employees SET start_date = ?, updated_at = ? WHERE employee_id = ?").bind(fixedDate, ts, r.employee_id).run();
    fixed += 1;
  }
  if (fixed) await logAdminAction(env, actor, "fix_buddhist_dates", "employees", `แปลง start_date ปี พ.ศ. → ค.ศ. จำนวน ${fixed} รายการ`);
  return json({ ok: true, fixed, scanned: rows.length });
}
...
async function handleRoute(request, env) {
  ...
  if (path === "/registerUser") return registerUser(request, env);
      ...
      if (path === "/adminFixBuddhistDates") return adminFixBuddhistDates(request, env);
