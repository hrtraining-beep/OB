...
// Self-skipping (module flag + schema check), atomic via exec() with defer_foreign_keys so the
// FK-referenced users table can be rebuilt. If it ever fails it changes nothing and retries.
...
async function migrateUsersEmailOptional(env) {
  ...
  try {
    ...
    await env.DB.exec([
      ...
      "CREATE TABLE users_new (user_id TEXT PRIMARY KEY, line_user_id TEXT UNIQUE, display_name TEXT, name TEXT NOT NULL, role TEXT NOT NULL, department TEXT NOT NULL, position TEXT, email TEXT, active INTEGER NOT NULL DEFAULT 1, created_at TEXT NOT NULL, updated_at TEXT NOT NULL, last_login_at TEXT, employee_id TEXT)",
      "INSERT INTO users_new (user_id, line_user_id, display_name, name, role, department, position, email, active, created_at, updated_at, last_login_at, employee_id) SELECT user_id, line_user_id, display_name, name, role, department, position, email, active, created_at, updated_at, last_login_at, employee_id FROM users",
      "DROP TABLE users",
    ...
    console.log("users.email migration failed (will retry next request):", String(error));
  }
}
