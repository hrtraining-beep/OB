...
async function registerUser(request, env) {
  ...
  const verifiedEmail = clean(verified.email).toLowerCase();
...
async function webLoginExchange(request, env) {
  ...
  await logAdminAction(env, user, "login", user.user_id, "เข้าสู่ระบบผ่านเว็บ (HR)");
...
async function getTask(request, env) {
  ...
  if (task.task_type === "Probation") {
    ...
    const supervisor = probationCase && probationCase.evaluator_user_id
      ...
      : null;
    const dayNo = [null, 30, 60, 90][Number(task.month_no || 0)] || null;
...
function corsOrigin(request, env) {
  ...
  try { if (/\.pages\.dev$/.test(new URL(origin).hostname)) return origin; } catch (e) {}
  return list[0];
}
