...
async function exportBackup(request, env) {
  ...
  const tables = ["users", "employees", "probation_cases", "probation_templates", "probation_self_reviews", "tasks", "checkpoints", "onboarding_groups", "group_members", "message_templates", "admin_logs", "app_settings", "line_bind_logs", "import_batches", "mentors"];
  const dump = {};
...
async function handleRoute(request, env) {
  ...
  if (path.startsWith("/kpi/") || path.startsWith("/360/")) {
    const ctx = { json, clean, nowIso, addDays, nextId, isOwner, requireAdmin, requireViewer, resolveActor, logAdminAction, getUserById, readJson };
