...
async function getPortal(request, env) {
  ...
  if (user.employee_id) {
    const kase = await env.DB.prepare(
      ...
      "LEFT JOIN users su ON su.user_id = pc.evaluator_user_id WHERE pc.employee_id = ? LIMIT 1"
...
async function overrideProbationPass(request, env) {
  ...
  const existing = await env.DB.prepare("SELECT case_id FROM probation_cases WHERE employee_id = ? LIMIT 1").bind(employeeId).first();
  if (existing) {
...
async function exportBackup(request, env) {
  ...
  // Keep in sync when a module adds a table — this dump is the only backup HR can take without
  // the wrangler CLI. Missing table = silently absent from every backup (per-table try/catch below).
  const tables = [
    "users", "employees", "probation_cases", "probation_templates", "probation_self_reviews", "tasks",
    "checkpoints", "onboarding_groups", "group_members", "message_templates", "admin_logs", "app_settings",
    "line_bind_logs", "import_batches", "mentors", "record_archives",
    "kpi_cycles", "kpi_corporate", "kpi_department_assignments", "kpi_department_heads", "kpi_grade_criteria",
    "kpi_individual_assignments", "kpi_sub", "kpi_progress",
    "feedback_360_cycles", "feedback_360_question_bank", "feedback_360_assignments", "feedback_360_responses",
    "feedback_360_gap_reports"
  ];
  const dump = {};
  for (const t of tables) {
    try { const r = await env.DB.prepare(`SELECT * FROM ${t}`).all(); dump[t] = r.results || []; }
    catch (e) { dump[t] = { error: String(e.message || e) }; }
  }
  return json({ ok: true, exportedAt: nowIso(), database: "nose-tea-onboarding", tables: dump });
}

// Owner-only reader for the append-only archive. Owner-only (not HR) to match /adminData's audit
// log: these snapshots hold raw pre-edit values, including 360 answers WITH their evaluator —
// which the anonymity model never exposes to anyone but the system owner.
...
async function archiveList(request, env) {
  ...
  const totalRow = await env.DB.prepare("SELECT COUNT(*) AS n FROM record_archives").first();
  return json({
    ok: true,
...
async function loadOwnProbationRound(env, input) {
  ...
  const kase = await env.DB.prepare("SELECT template_id FROM probation_cases WHERE employee_id = ? LIMIT 1").bind(user.employee_id).first();
...
async function handleRoute(request, env) {
  ...
  if (path.startsWith("/kpi/") || path.startsWith("/360/")) {
    ...
    const ctx = { json, clean, nowIso, addDays, nextId, isOwner, requireAdmin, requireViewer, resolveActor, logAdminAction, getUserById, readJson, ensureRecordArchives, archiveStmt, archiveSnapshot };
