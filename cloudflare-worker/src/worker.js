...
async function getTask(request, env) {
  ...
  if (task.task_type === "Probation") {
    ...
    const employee = await env.DB.prepare("SELECT employee_name, position, start_date FROM employees WHERE employee_id = ? LIMIT 1").bind(task.employee_id).first();
    ...
    probation = {
      ...
      day: [null, 30, 60, 90][Number(task.month_no || 0)] || null,
      prevSubmission
    };
  }
...
async function adminData(request, env) {
  ...
  const [users, employees, importBatches, bindLogs, tasks, checkpoints, notifications, templates, groups, groupMembers, probationCases, probationTemplates] = await Promise.all([
    ...
    env.DB.prepare(`
      ...
      LIMIT 200
    `).all(),
    env.DB.prepare("SELECT * FROM probation_templates WHERE active = 1 ORDER BY template_id ASC").all()
  ]);
...
async function saveProbationTemplate(request, env) {
  const { input } = await requireAdmin(request, env);
  await ensureProbationTemplates(env);
  const templateId = clean(input.templateId) || await nextId(env, "PT");
  const name = clean(input.name);
...
async function deleteProbationTemplate(request, env) {
  ...
  await ensureProbationTemplates(env);
  await env.DB.prepare("UPDATE probation_templates SET active = 0, updated_at = ? WHERE template_id = ?").bind(nowIso(), templateId).run();
  return json({ ok: true });
}
...
async function resolveActor(env, input) {
  ...
  const verified = await resolveLineIdentity(env, input);
  return getUserByLine(env, verified.sub);
}
