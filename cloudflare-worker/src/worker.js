...
async function registerUser(request, env) {
  ...
  `).bind(
    ...
    lineUserId,
    verified.name || input.lineDisplayName || effName,
    effName,
    finalRole,
    effDepartment,
    effPosition,
...
async function webLoginExchange(request, env) {
  ...
  if (!user) throw new Error("บัญชี LINE นี้ยังไม่ได้ลงทะเบียน กรุณาเปิดผ่านแอป LINE เพื่อลงทะเบียนก่อน");
  if (!user.active) throw new Error("บัญชีนี้ถูกปิดใช้งาน กรุณาติดต่อ HR");
