...
async function execSummary(request, env) {
  ...
  const [emps, cases, openCnt, overdueCnt, obTasks, probTasks] = await Promise.all([
    ...
    env.DB.prepare("SELECT employee_id, result FROM probation_cases").all(),
...
async function assignProbationSupervisor(request, env) {
  ...
  if (existing && clean(existing.template_id) && clean(existing.template_id) !== templateId) {
    const done = await env.DB.prepare("SELECT COUNT(*) AS n FROM tasks WHERE task_type = 'Probation' AND employee_id = ? AND status = 'Completed'").bind(employeeId).first();
...
// One-time auto-migration (no CLI needed): make users.email OPTIONAL + NON-UNIQUE so staff can
// share an inbox / use gmail / have no email. Identity stays on line_user_id + employee code.
// Self-skipping (module flag + schema check). Runs as ONE batch() transaction with
// defer_foreign_keys so the FK-referenced users table can be rebuilt; if it fails it changes
// nothing (atomic rollback) and retries. Check progress at GET /emailMigrationStatus.
let usersEmailMigrationDone = false;
let usersEmailMigrationError = "";
async function migrateUsersEmailOptional(env) {
  if (usersEmailMigrationDone) return;
  try {
    const row = await env.DB.prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='users'").first();
    if (!row || !/email[^,]*UNIQUE/i.test(row.sql || "")) { usersEmailMigrationDone = true; return; }
    await env.DB.batch([
      env.DB.prepare("PRAGMA defer_foreign_keys = TRUE"),
      env.DB.prepare("DROP TABLE IF EXISTS users_new"),
      env.DB.prepare("CREATE TABLE users_new (user_id TEXT PRIMARY KEY, line_user_id TEXT UNIQUE, display_name TEXT, name TEXT NOT NULL, role TEXT NOT NULL, department TEXT NOT NULL, position TEXT, email TEXT, active INTEGER NOT NULL DEFAULT 1, created_at TEXT NOT NULL, updated_at TEXT NOT NULL, last_login_at TEXT, employee_id TEXT)"),
      env.DB.prepare("INSERT INTO users_new (user_id, line_user_id, display_name, name, role, department, position, email, active, created_at, updated_at, last_login_at, employee_id) SELECT user_id, line_user_id, display_name, name, role, department, position, email, active, created_at, updated_at, last_login_at, employee_id FROM users"),
      env.DB.prepare("DROP TABLE users"),
      env.DB.prepare("ALTER TABLE users_new RENAME TO users"),
      env.DB.prepare("CREATE INDEX IF NOT EXISTS idx_users_line_user_id ON users(line_user_id)"),
      env.DB.prepare("CREATE INDEX IF NOT EXISTS idx_users_email ON users(email)"),
      env.DB.prepare("CREATE INDEX IF NOT EXISTS idx_users_employee_id ON users(employee_id)")
    ]);
    usersEmailMigrationDone = true;
    usersEmailMigrationError = "";
    console.log("users.email migration: applied");
  } catch (error) {
    usersEmailMigrationError = String((error && error.message) || error);
    console.log("users.email migration failed:", usersEmailMigrationError);
  }
}
...
async function handleRoute(request, env) {
  ...
  if (path === "/emailMigrationStatus") {
    await migrateUsersEmailOptional(env);
    const row = await env.DB.prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='users'").first();
    const migrated = !!(row && !/email[^,]*UNIQUE/i.test(row.sql || ""));
    return json({ ok: true, migrated, lastError: usersEmailMigrationError || null, usersSql: (row && row.sql) || null });
  }
  await migrateUsersEmailOptional(env);
