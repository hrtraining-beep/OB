...
async function fb360People(env) {
  const res = await env.DB.prepare(`
    SELECT employee_id, employee_name, level, rank, department, user_id
    FROM employees
...
async function fb360ReportOverview(request, env, ctx) {
  ...
  if (!cycleId) return ctx.json({ ok: true, cycles, cycleId: null, subjects: [], climate: { responders: 0 }, weights, minN });
  const people = await fb360People(env);
  ...
  return ctx.json({ ok: true, cycles, cycleId, subjects, climate: { responders: (climR && climR.n) || 0 }, weights, minN });
}
...
async function fb360ReportSubject(request, env, ctx) {
  ...
  const themes = Object.keys(agg).map(t => {
    const g = agg[t];
    const avg = rel => (g[rel] && g[rel].n) ? g[rel].sum / g[rel].n : null;
  ...
  const comments = (cRes.results || []).map(r => ({ rel: r.rel, text: r.txt }));   // no evaluator id → anonymous
  const counts = { self: responders.self.size, peer: responders.peer.size, boss: responders.boss.size };
...
async function fb360SettingsSave(request, env, ctx) {
  ...
  await ctx.logAdminAction(env, user, "fb360_settings_save", "weights", `${boss}/${peer}/${self} minN=${minN}`);
  return ctx.json({ ok: true });
}
