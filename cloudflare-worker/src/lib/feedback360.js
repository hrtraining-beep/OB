...
async function ensureFb360Tables(env, ctx) {
  ...
  // Was: seed all 60 questions on EVERY /360/* request (5,146 no-op INSERTs = 18% of all D1
  // runtime, 0 rows written, and ~74 queries per request vs the 50-subrequest Free cap).
  // Bump the version below if SEED_QUESTIONS changes — the set then re-seeds once.
  await ctx.seedOnce(env, "seed_360_questions", "1", () => seedQuestionBank(env));
}
...
export async function handle360(path, request, env, ctx) {
  await ensureFb360Tables(env, ctx);
...
async function fb360Overview(request, env, ctx) {
  ...
  const cyclesRes = await env.DB.prepare("SELECT * FROM feedback_360_cycles ORDER BY cycle_year DESC, CASE cycle_period WHEN 'H2' THEN 0 WHEN 'H1' THEN 1 ELSE 2 END").all();
...
// Archive real answers that a pairing/lifecycle change is about to destroy (append-only audit).
// `where` filters the responses⋈assignments join. Nothing matched → no-op, which is the normal
// case for the draft-stage paths (answers only exist once a cycle is active).
async function fb360ArchiveResponses(env, ctx, where, binds, entityId, reason, actor) {
  const res = await env.DB.prepare(
    "SELECT r.response_id, r.assignment_id, r.question_id, r.theme, r.score, r.open_text, r.submitted_at, " +
    "a.cycle_id, a.relation_type, a.evaluator_user_id, a.subject_user_id " +
    "FROM feedback_360_responses r JOIN feedback_360_assignments a ON a.assignment_id = r.assignment_id " +
    "WHERE " + where).bind(...binds).all();
  const rows = res.results || [];
  if (!rows.length) return 0;
  await ctx.archiveSnapshot(env, {
    entityType: "feedback_360_responses", entityId, employeeId: null, action: "delete",
    reason, snapshot: { answers: rows }, actor
  });
  return rows.length;
}
...
async function fb360CycleDelete(request, env, ctx) {
  ...
  // Audit: a cycle can be deleted while it holds real answers — archive them before the cascade.
  await fb360ArchiveResponses(env, ctx, "a.cycle_id = ?", [cycleId], cycleId, "ลบรอบประเมิน 360", user);
...
async function fb360People(env) {
  // branch/job_group ride along for the pairing filters — same row, no extra query. Pairing is the
  // one screen that must pick from the WHOLE org (122 → 422 people), so it needs every axis HR
  // thinks in: department (= สาขา once branches roll out), branch, Store/Office.
  const res = await env.DB.prepare(`
    SELECT employee_id, employee_name, level, rank, department, branch, job_group, user_id, start_date
    FROM employees
  ...
  return (res.results || []).map(r => ({
    ...
    branch: r.branch || "", jobGroup: r.job_group || "",
...
async function fb360Generate(request, env, ctx) {
  ...
  // Audit: re-generating pairings wipes the assignments (and any answers already given).
  await fb360ArchiveResponses(env, ctx, "a.cycle_id = ?", [cycleId], cycleId, "สร้างคู่ประเมินใหม่ (ทับของเดิม)", user);
  await env.DB.prepare("DELETE FROM feedback_360_responses WHERE assignment_id IN (SELECT assignment_id FROM feedback_360_assignments WHERE cycle_id = ?)").bind(cycleId).run();
  await env.DB.prepare("DELETE FROM feedback_360_assignments WHERE cycle_id = ?").bind(cycleId).run();
  const ts = ctx.nowIso();
...
async function fb360PairingList(request, env, ctx) {
  ...
  return ctx.json({
    ...
    people: people.map(p => ({ id: p.id, name: p.name, department: p.department, level: p.level, rank: p.rank, branch: p.branch, jobGroup: p.jobGroup }))
  });
}
...
async function fb360SetBoss(request, env, ctx) {
  ...
  const ts = ctx.nowIso();
  await fb360ArchiveResponses(env, ctx, "a.cycle_id = ? AND a.relation_type = 'boss' AND a.evaluator_user_id = ?", [cycleId, subordinate], cycleId, "เปลี่ยนหัวหน้าผู้ถูกประเมิน", user);
...
async function fb360RemoveAssignment(request, env, ctx) {
  ...
  await fb360ArchiveResponses(env, ctx, "r.assignment_id = ?", [assignmentId], assignmentId, "ลบคู่ประเมินออกจากรอบ", user);
...
async function fb360Form(request, env, ctx) {
  ...
  return ctx.json({
    ...
    questions, answers,
    scaleLabels: await fb360ScaleLabels(env)   // captions ride with the form so HR edits apply at once
  });
}
...
async function fb360Submit(request, env, ctx) {
  ...
  // Audit: an edit-after-submit replaces the whole answer set. Archive the outgoing set INSIDE the
  // same batch, so the old answers can never be dropped while the new ones land.
  const priorRes = await env.DB.prepare("SELECT question_id, theme, score, open_text, submitted_at FROM feedback_360_responses WHERE assignment_id = ?").bind(assignmentId).all();
  const prior = priorRes.results || [];
  const batch = [];
  if (prior.length) {
    await ctx.ensureRecordArchives(env);
    batch.push(ctx.archiveStmt(env, {
      entityType: "feedback_360_responses", entityId: assignmentId, employeeId: a.subject_user_id, action: "overwrite",
      reason: `แก้คำตอบ 360 หลังส่ง (${a.relation_type})`,
      snapshot: { cycleId: a.cycle_id, relation: a.relation_type, subjectId: a.subject_user_id, answers: prior },
      actor: user
    }));
  }
  // Atomic overwrite: clear prior answers + insert the fresh set in one batch (D1 batch = transaction).
  batch.push(env.DB.prepare("DELETE FROM feedback_360_responses WHERE assignment_id = ?").bind(assignmentId), ...inserts);
...
// HR-editable captions for the 1-5 buttons on the answer form. Mirrors ob_feedback_labels: stored
// in app_settings, blank slot = fall back to the standard word, so HR can override just one level.
// The caption is display only — the stored score is always the number.
const FB360_SCALE_DEFAULT = ["ต้องปรับปรุง", "ควรพัฒนา", "ตามที่คาดหวัง", "สูงกว่าคาดหวัง", "ดีเยี่ยม"];
function fb360MergeScaleLabels(arr) {
  const list = Array.isArray(arr) ? arr : [];
  return FB360_SCALE_DEFAULT.map((def, i) => {
    const v = list[i] == null ? "" : String(list[i]).trim();
    return v || def;
  });
}
async function fb360ScaleLabels(env) {
  const row = await env.DB.prepare("SELECT value FROM app_settings WHERE key = 'feedback_360_scale_labels'").first();
  if (!row || !row.value) return FB360_SCALE_DEFAULT.slice();
  try { return fb360MergeScaleLabels(JSON.parse(row.value)); }
  catch (e) { return FB360_SCALE_DEFAULT.slice(); }
}
...
async function fb360Weights(env) {
  ...
  try {
    ...
    // A weight HR deliberately set to 0 (e.g. exclude self-assessment) must stay 0.
    // `Number(x) || def` would treat 0 as missing and silently restore the default.
    const w = (x, d) => { const n = Number(x); return Number.isFinite(n) ? n : d; };
    return { boss: w(v.boss, def.boss), peer: w(v.peer, def.peer), self: w(v.self, def.self) };
  } catch (e) { return def; }
}
...
async function fb360ReportOverview(request, env, ctx) {
  ...
  const cycRes = await env.DB.prepare("SELECT * FROM feedback_360_cycles ORDER BY cycle_year DESC, CASE cycle_period WHEN 'H2' THEN 0 WHEN 'H1' THEN 1 ELSE 2 END").all();
  ...
  const cutoffDays = await fb360CutoffDays(env);
  const scaleLabels = await fb360ScaleLabels(env);
  if (!cycleId) return ctx.json({ ok: true, cycles, cycleId: null, subjects: [], climate: { responders: 0 }, weights, minN, cutoffDays, scaleLabels });
  const people = await fb360People(env);
  ...
  return ctx.json({ ok: true, cycles, cycleId, subjects, climate: { responders: (climR && climR.n) || 0 }, weights, minN, cutoffDays, scaleLabels });
}
...
async function fb360MyReport(request, env, ctx) {
  ...
  for (const r of (repRes.results || [])) {
    ...
    // boss is EXEMPT from min-N: a subject has exactly one boss, so counts.boss can never reach the
    // default minN=3 and the boss axis (weight ~0.4) would be hidden forever, making the employee-facing
    // overall diverge from HR's. min-N exists to stop reverse-identifying raters in a GROUP — with a
    // single, already-known boss there is nothing to anonymize. peer/self keep the min-N mask.
    const allow = { self: true, peer: counts.peer >= minN, boss: counts.boss >= 1 };
    ...
    reports.push({
      ...
      counts: { peer: counts.peer, boss: counts.boss },
...
async function fb360Climate(request, env, ctx) {
  const { input } = await ctx.requireViewer(request, env);
  let cycleId = ctx.clean(input.cycleId);
  // Ordering by year alone left NO tiebreak between H1 and H2 of the same year, and a cycle stays
  // 'active' when the next one opens (line ~534 never clears the old) — so with both halves active
  // SQLite was free to return either row, and could return a different one on a later call.
  if (!cycleId) { const c = await env.DB.prepare("SELECT cycle_id FROM feedback_360_cycles WHERE status = 'active' ORDER BY cycle_year DESC, CASE cycle_period WHEN 'H2' THEN 0 WHEN 'H1' THEN 1 ELSE 2 END LIMIT 1").first(); cycleId = c ? c.cycle_id : null; }
...
async function fb360SettingsSave(request, env, ctx) {
  ...
  // Optional: only written when the client sends it, so an older form can't wipe the captions.
  if (Array.isArray(input.scaleLabels)) {
    const labels = input.scaleLabels.slice(0, 5).map(v => ctx.clean(v));
    await env.DB.prepare("INSERT INTO app_settings(key, value, updated_at) VALUES('feedback_360_scale_labels', ?, ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value, updated_at = excluded.updated_at")
      .bind(JSON.stringify(labels), ts).run();
  }
...
async function fb360ReportGroup(request, env, ctx) {
  ...
  // PRIVACY: per-person 360 results are HR-only (fb360ReportSubject = requireAdmin). This group
  // view is readable by Executives (requireViewer), so when a chosen department has too few subjects
  // the "aggregate" would actually expose one person's profile. Mask the per-theme self/peer/boss
  // numbers below the min-responses floor. Whole-org (no dept filter) is always large enough.
  const minSubjects = await fb360MinResponses(env);
  const masked = Boolean(dept) && subjects.size < minSubjects;
  const themes = masked ? [] : fb360BuildThemes(agg, weights, { self: true, peer: true, boss: true });
  ...
  const rr = await env.DB.prepare("SELECT COUNT(DISTINCT evaluator_user_id) AS n FROM feedback_360_assignments WHERE cycle_id = ? AND relation_type = 'org' AND status = 'completed'").bind(cycleId).first();
  return ctx.json({ ok: true, cycles, cycleId, department: dept || null, departments, themes, subjectCount: subjects.size, masked, minSubjects, climate, climateResponders: (rr && rr.n) || 0 });
}
