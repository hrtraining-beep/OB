...
// users.email is UNIQUE NOT NULL in the live DB and can't be migrated (D1 won't defer the
// foreign keys that reference users, and CLI access is blocked). So email is treated as optional
// contact info: when blank or already taken we store a hidden unique placeholder to satisfy the
// constraint; these helpers create/detect it and publicUser() shows it as blank.
export function isPlaceholderEmail(email) {
  return /@noemail\.local$/i.test(String(email || ""));
}

export function placeholderEmail(userId) {
  return `${userId}@noemail.local`;
}
