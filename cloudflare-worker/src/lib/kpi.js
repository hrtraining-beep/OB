...
async function kpiMyList(request, env, ctx) {
  ...
  const res = await env.DB.prepare(`
    SELECT s.*, k.title AS parent_title
    FROM kpi_sub s LEFT JOIN kpi_corporate k ON k.kpi_id = s.parent_kpi_id
