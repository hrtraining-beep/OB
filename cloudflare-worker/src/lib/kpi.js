...
// Newest cycle first, by TIME rather than by spelling.
//
// `ORDER BY cycle_period ASC` sorted the period as a string, so within a year it ran
// 'Full' → 'H1' → 'H2' — i.e. the EARLIEST half won, the exact opposite of the "default = latest"
// this was documented as. It stayed invisible while only H1 existed and bit the moment H2 opened:
// kpiPublish marks the new cycle 'active' without clearing the old one, so both halves are active
// at once and `cycles.find(status === 'active')` picked H1 — the half that had already finished —
// and kept picking it. An executive would read last half's numbers off a dashboard that looked fine.
//
// H2 after H1 within a year; 'Full' spans the year, so it sorts last on the rare occasion it
// coexists with halves (in practice nobody runs a single full-year cycle).
const CYCLE_ORDER = "ORDER BY cycle_year DESC, CASE cycle_period WHEN 'H2' THEN 0 WHEN 'H1' THEN 1 WHEN 'Full' THEN 2 ELSE 3 END";
...
async function kpiOverview(request, env, ctx) {
  ...
  const cyclesRes = await env.DB.prepare(`SELECT * FROM kpi_cycles ${CYCLE_ORDER}`).all();
...
async function kpiDeptOverview(request, env, ctx) {
  ...
  const cyc = await env.DB.prepare("SELECT * FROM kpi_cycles WHERE status = 'active' ORDER BY cycle_year DESC, CASE cycle_period WHEN 'H2' THEN 0 WHEN 'H1' THEN 1 WHEN 'Full' THEN 2 ELSE 3 END LIMIT 1").first();
...
async function kpiSubSave(request, env, ctx) {
  ...
  if (parentKpiId) {
    ...
    const cyc = await env.DB.prepare("SELECT cycle_id FROM kpi_cycles WHERE status = 'active' ORDER BY cycle_year DESC, CASE cycle_period WHEN 'H2' THEN 0 WHEN 'H1' THEN 1 WHEN 'Full' THEN 2 ELSE 3 END LIMIT 1").first();
...
async function kpiMySubmit(request, env, ctx) {
  ...
  if (!subId) throw new Error("subId จำเป็น");
  const sub = await env.DB.prepare("SELECT user_id, approved_at, department, title, actual_result, self_grade, self_submitted_at FROM kpi_sub WHERE sub_id = ?").bind(subId).first();
  ...
  const ts = ctx.nowIso();
  // Audit: this row is editable until approval, so a re-submit destroys what was reported before.
  // Archive the previous self-reported result/grade first (append-only). Skips the first submit.
  if (sub.self_submitted_at) {
    await ctx.archiveSnapshot(env, {
      entityType: "kpi_sub_self", entityId: subId, employeeId: user.employee_id, action: "overwrite",
      reason: "พนักงานแก้ผล/เกรดตัวเอง (KPI)",
      snapshot: { title: sub.title, department: sub.department, actualResult: sub.actual_result, selfGrade: sub.self_grade, selfSubmittedAt: sub.self_submitted_at },
      actor: user
    });
  }
...
async function kpiProgressSave(request, env, ctx) {
  ...
  const ts = ctx.nowIso();
  // Audit: the ON CONFLICT below overwrites a month that was already reported. Keep what it said.
  const priorMonth = await env.DB.prepare("SELECT value, note, updated_at FROM kpi_progress WHERE sub_id = ? AND month_no = ?").bind(subId, monthNo).first();
  if (priorMonth) {
    await ctx.archiveSnapshot(env, {
      entityType: "kpi_progress", entityId: subId, employeeId: user.employee_id, action: "overwrite",
      reason: `แก้ยอดรายเดือน (เดือนที่ ${monthNo})`,
      snapshot: { monthNo, value: priorMonth.value, note: priorMonth.note, updatedAt: priorMonth.updated_at },
      actor: user
    });
  }
...
async function kpiSubApprove(request, env, ctx) {
  ...
  const subId = ctx.clean(hc.input.subId);
  if (!subId) throw new Error("subId จำเป็น");
  const sub = await env.DB.prepare("SELECT department, title, user_id, actual_result, self_grade, final_grade, approved_by, approved_at FROM kpi_sub WHERE sub_id = ?").bind(subId).first();
  if (!sub) throw new Error("ไม่พบ KPI");
  assertHeadOf(hc, sub.department);
  // Audit: the final grade is the number that reaches the corporate dashboard. Changing or
  // cancelling an approval overwrites it — archive the approved grade before it goes.
  if (sub.approved_at) {
    await ctx.archiveSnapshot(env, {
      entityType: "kpi_sub_grade", entityId: subId, employeeId: null, action: "overwrite",
      reason: ctx.clean(hc.input.finalGrade) ? "หัวหน้าแก้เกรดที่อนุมัติแล้ว" : "หัวหน้ายกเลิกการอนุมัติเกรด",
      snapshot: { title: sub.title, department: sub.department, userId: sub.user_id, actualResult: sub.actual_result, selfGrade: sub.self_grade, finalGrade: sub.final_grade, approvedBy: sub.approved_by, approvedAt: sub.approved_at },
      actor: hc.user
    });
  }
...
async function kpiDashboard(request, env, ctx) {
  ...
  if (!cycleId) {
    // CYCLE_ORDER is newest-first, so "the first active one" is now the LATEST active one — which
    // matters because publishing H2 leaves H1 active too (kpiPublish never clears the old cycle).
