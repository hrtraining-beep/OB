...
(function () {
  ...
  function renderAssignDept(kpi) {
    ...
    const rows = state.departments.map(d => `
      <label class="m3-eitem" style="justify-content:space-between;cursor:pointer">
        <span>${esc(d)}</span>
        <input type="checkbox" class="kAsgDept" value="${esc(d)}" ${assigned.has(d) ? 'checked' : ''}>
      </label>`).join('');
    NT.render(NT.m3Shell('profile', `
  ...
  async function renderDeptHome() {
    NT.render(NT.m3Shell('profile', `<section class="m3-section"><div class="m3-card m3-card-pad">${mi('sync')} กำลังโหลด KPI ฝ่ายของคุณ...</div></section>`, bar('KPI ฝ่ายของฉัน')));
  ...
  function paintDept() {
    ...
    document.querySelectorAll('[data-crit]').forEach(b => b.addEventListener('click', () => {
      const dep = d.departments.find(x => x.department === b.dataset.dep);
      renderCriteriaForm(dep, dep.kpis.find(k => k.kpiId === b.dataset.crit));
    }));
    document.querySelectorAll('[data-dist]').forEach(b => b.addEventListener('click', () => {
      const dep = d.departments.find(x => x.department === b.dataset.dep);
      renderDistribute(dep, dep.kpis.find(k => k.kpiId === b.dataset.dist));
    }));
  }

  function deptKpiCard(dep, k) {
    const critDone = k.criteria && k.criteria.every(v => String(v || '').trim());
    const assigned = (k.assignedUserIds || []).length;
    return `<div class="m3-card m3-card-pad" style="margin-bottom:10px">
      <div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start">
      ...
      <div style="margin-top:8px;display:flex;flex-wrap:wrap;gap:4px">
        <span class="m3-badge ${critDone ? 'm3-badge--ok' : 'm3-badge--warn'}">${critDone ? 'ตั้งเกณฑ์แล้ว' : 'ยังไม่ตั้งเกณฑ์'}</span>
        <span class="m3-badge ${assigned ? 'm3-badge--ok' : ''}">แจกแล้ว ${assigned} คน</span>
      </div>
      <div class="m3-tpl-actions" style="margin-top:10px">
        <button type="button" class="m3-btn m3-btn--tonal" data-crit="${esc(k.kpiId)}" data-dep="${esc(dep.department)}">${mi('tune')}ตั้งเกณฑ์ให้คะแนน</button>
        <button type="button" class="m3-btn m3-btn--ghost" data-dist="${esc(k.kpiId)}" data-dep="${esc(dep.department)}" ${critDone ? '' : 'disabled'}>${mi('group_add')}แจกให้พนักงาน</button>
      </div>
      ${critDone ? '' : '<p class="m3-save-hint" style="text-align:left;margin:6px 0 0">ตั้งเกณฑ์ให้คะแนนครบ 5 ระดับก่อน จึงจะแจกให้พนักงานได้</p>'}
    </div>`;
  }
  ...
  function renderDistribute(dep, k) {
    ...
    const rows = cands.length ? cands.map(u => `
      <label class="m3-eitem" style="justify-content:space-between;cursor:pointer">
        <span>${esc(u.name)}</span>
        <input type="checkbox" class="kDist" value="${esc(u.userId)}" ${assigned.has(u.userId) ? 'checked' : ''}>
      </label>`).join('') : '<div class="m3-empty">ยังไม่มีพนักงานที่ลงทะเบียนในฝ่ายนี้</div>';
    NT.render(NT.m3Shell('profile', `
  ...
  window.KPI = { renderHome, renderDeptHome };
})();
