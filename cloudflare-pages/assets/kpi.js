...
(function () {
    ...
    NT.render(NT.m3Shell('profile', `
      <section class="m3-section" style="gap:8px">
        ...
        <div style="display:flex;gap:8px;align-items:center">
          <select class="m3-select" id="kCycleSel" style="flex:1">${state.cycles.map(c => `<option value="${esc(c.cycleId)}" ${c.cycleId === state.cycleId ? 'selected' : ''}>${esc(cycleLabel(c))}${c.status !== 'draft' ? ' · ' + esc(c.status) : ''}</option>`).join('')}</select>
          <button type="button" class="m3-btn m3-btn--ghost" id="kNewCycle" style="min-height:44px">${mi('add')}รอบใหม่</button>
        </div>
  ...
  function kpiCard(k) {
    const code = [k.objectiveCode, k.kpiCode].filter(Boolean).join(' · ');
    return `
      <div class="m3-card m3-card-pad" style="margin-bottom:10px">
        <div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start">
          <div style="flex:1;min-width:0">
            <div class="m3-staff-name">${esc(k.title)}</div>
            <div class="m3-staff-role">${code ? esc(code) + ' · ' : ''}${k.target ? 'เป้า ' + esc(k.target) + (k.unit ? ' ' + esc(k.unit) : '') : 'ยังไม่ตั้งเป้า'}${k.deadline ? ' · ' + esc(NT.formatThaiDate(k.deadline)) : ''}</div>
          </div>
  ...
  function renderKpiForm(kpi) {
    ...
    NT.render(NT.m3Shell('profile', `
      ...
      <form id="kKpiForm" class="m3-section">
        ...
        <label class="m3-elabel">Perspective (Balanced Scorecard)</label>
        <select class="m3-select" name="perspective"><option value="">— เลือก —</option>${state.perspectives.map(p => `<option value="${esc(p)}" ${k.perspective === p ? 'selected' : ''}>${esc(p)}</option>`).join('')}</select>
        <div style="display:flex;gap:10px">
          <div style="flex:1"><label class="m3-elabel">รหัส Objective</label><input class="m3-input" name="objectiveCode" value="${esc(k.objectiveCode || '')}" placeholder="เช่น K1"></div>
          <div style="flex:1"><label class="m3-elabel">รหัส KPI</label><input class="m3-input" name="kpiCode" value="${esc(k.kpiCode || '')}" placeholder="เช่น K1.1"></div>
        </div>
        <label class="m3-elabel">ชื่อ / เป้าหมาย (Objective)</label><input class="m3-input" name="title" required value="${esc(k.title || '')}" placeholder="เช่น เพิ่มยอดขายรวม">
