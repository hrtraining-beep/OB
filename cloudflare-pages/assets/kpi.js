...
(function () {
  ...
  async function renderDashboard() {
    ...
    const totalCard = `<div class="m3-card m3-card-pad" style="text-align:center;border-color:var(--m3-primary)">
      ...
      <div class="m3-save-hint" style="margin-top:2px">อนุมัติแล้ว ${scored.length}/${rows.length} KPI · เต็มทั้งรอบ = 100</div>
    </div>`;
    const kpiRow = k => {
      ...
      return `<div class="m3-card m3-card-pad" style="margin-bottom:10px">
        <div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start">
          <div style="flex:1;min-width:0"><div class="m3-staff-name">${esc(k.title)}</div><div class="m3-staff-role">น้ำหนัก ${k.weight}%${k.pct != null ? ' · สมทบ ' + k.contribution + ' แต้ม' : ''}</div></div>
    ...
    NT.render(NT.m3Shell('profile', `
      ...
      <section class="m3-section">${totalCard}</section>
      <section class="m3-section">${body}</section>
