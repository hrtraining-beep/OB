...
(function () {
  ...
  function paintGroupReport() {
    ...
    const themeCards = d.masked
      ? `<div class="m3-empty">ปกปิดข้อมูลเพื่อความเป็นส่วนตัว — กลุ่มนี้มีผู้ถูกประเมินน้อยกว่า ${d.minSubjects || 3} คน จึงแสดงคะแนนรวมรายหมวดไม่ได้</div>`
      : ((d.themes || []).length ? d.themes.map(themeCard).join('') : '<div class="m3-empty">ยังไม่มีคะแนน</div>');
