...
(function () {
  ...
  function paint() {
    ...
    NT.render(NT.m3Shell('profile', `
      ...
      <section class="m3-section">
        ...
        <button type="button" class="m3-btn m3-btn--ghost" id="fbSettings">${mi('tune')}ตั้งค่าน้ำหนัก / ผู้ตอบขั้นต่ำ</button>
      </section>
  ...
  async function generatePairing() {
    ...
    try { const r = await NT.api('/360/pairing/generate', { cycleId: pstate.cycleId }); NT.toast(`สร้างคู่แล้ว ${r.count} รายการ`); pstate.view = 'hub'; await loadPairing(); paintPairing(); }
  ...
  async function renderSettings() {
    ...
    NT.wireM3Nav({ back: () => paint() });
    let w = { boss: 0.4, peer: 0.4, self: 0.2 }, minN = 3;
    try { const d = await NT.api('/360/report/overview', {}); w = d.weights || w; minN = d.minN || minN; } catch (e) { /* use defaults */ }
    NT.render(NT.m3Shell('profile', `
  ...
  function paintMyReport(reports) {
    if (!reports.length) {
      NT.render(NT.m3Shell('profile', `
        <section class="m3-section" style="gap:4px"><h2 class="m3-title">ผลประเมิน 360 ของฉัน</h2></section>
        <section class="m3-section"><div class="m3-empty">${mi('hourglass_empty')} ยังไม่มีผลที่เผยแพร่<br><span style="font-size:12px">เมื่อ HR สรุปและเผยแพร่ คุณจะเห็นผลของตัวเองที่นี่</span></div></section>`, bar('ผลประเมิน 360 ของฉัน')));
      NT.wireM3Nav({ back: () => NT.goPortal() });
      return;
    }
    const body = reports.map(r => {
      const summary = r.summary ? `<div class="m3-card m3-card-pad" style="white-space:pre-line;line-height:1.7;border-color:var(--m3-primary)">${esc(r.summary)}</div>` : '';
      const themeCards = (r.themes || []).length ? r.themes.map(themeCard).join('') : '<div class="m3-empty">ยังไม่มีคะแนน</div>';
      const hiddenNote = (r.hidden && (r.hidden.peer || r.hidden.boss)) ? `<p class="m3-save-hint" style="text-align:left">* บางกลุ่มมีผู้ตอบน้อยกว่า ${r.minN} คน จึงซ่อนไว้เพื่อรักษาความลับของผู้ตอบ</p>` : '';
      const commentGroups = ['self', 'peer', 'boss'].map(rel => {
        const items = (r.comments || []).filter(c => c.rel === rel);
        if (!items.length) return '';
        return `<div class="m3-section-label" style="margin-top:12px">${RREL[rel]}</div>${items.map(c => `<div class="m3-card m3-card-pad" style="margin-bottom:6px;font-size:14px;line-height:1.5">${esc(c.text)}</div>`).join('')}`;
      }).join('');
      return `<section class="m3-section" style="gap:4px"><h3 class="m3-section-label">ปี ${r.year} · ${r.period}</h3></section>
        ${summary ? `<section class="m3-section">${summary}</section>` : ''}
        <section class="m3-section"><h3 class="m3-section-label">คะแนนต่อหมวด</h3>${themeCards}${hiddenNote}</section>
        ${commentGroups ? `<section class="m3-section"><h3 class="m3-section-label">ความคิดเห็น (ไม่ระบุผู้ให้)</h3>${commentGroups}</section>` : ''}`;
    }).join('');
    NT.render(NT.m3Shell('profile', `
      <section class="m3-section" style="gap:4px"><h2 class="m3-title">ผลประเมิน 360 ของฉัน</h2><p class="m3-eyebrow">คะแนนเฉลี่ย · ไม่ระบุว่าใครให้คะแนน</p></section>
      ${body}<div style="height:40px"></div>`, bar('ผลประเมิน 360 ของฉัน')));
    NT.wireM3Nav({ back: () => NT.goPortal() });
  }
