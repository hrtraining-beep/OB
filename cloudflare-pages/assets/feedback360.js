...
(function () {
  ...
  function personOptions(selected) {
    return (pstate.data.people || []).map(p => `<option value="${esc(p.userId)}" ${p.userId === selected ? 'selected' : ''}>${esc(p.name)}${p.department ? ' · ' + esc(p.department) : ''}</option>`).join('');
  }
  ...
  function paintPairing() {
    ...
    const noBossHtml = noBoss.length ? `<div class="m3-section-label" style="margin-top:14px;color:var(--m3-error)">⚠️ หาหัวหน้าไม่เจอ (${noBoss.length}) — เลือกให้เอง</div>` +
      noBoss.map(p => `<div class="m3-card m3-card-pad" style="margin-bottom:8px">
        ...
        <div style="display:flex;gap:8px;margin-top:8px">
          <select class="m3-select pBossSel" data-sub="${esc(p.userId)}" style="flex:1"><option value="">— เลือกหัวหน้า —</option>${personOptions('')}</select>
          <button type="button" class="m3-btn m3-btn--tonal pBossSet" data-sub="${esc(p.userId)}" style="width:auto;flex:none">${mi('check')}</button>
        </div></div>`).join('') : '';
    // boss list
    ...
    const bossHtml = `<div class="m3-section-label" style="margin-top:14px">หัวหน้า (BOSS) · ลูกน้อง → หัวหน้า (${bossRows.length})</div>` +
      (bossRows.length ? bossRows.map(b => `<div class="m3-card m3-card-pad" style="margin-bottom:8px">
        ...
        <div style="display:flex;gap:8px;margin-top:6px">
          <select class="m3-select pBossSel" data-sub="${esc(b.subordinateUserId)}" style="flex:1">${personOptions(b.bossUserId)}</select>
          <button type="button" class="m3-btn m3-btn--ghost pBossSet" data-sub="${esc(b.subordinateUserId)}" style="width:auto;flex:none">${mi('sync')}</button>
    ...
    const peerHtml = `<div class="m3-section-label" style="margin-top:14px">เพื่อนร่วมงาน (PEER) · ใครประเมินใคร (${peerGroups.length} คนถูกประเมิน)</div>` +
      (peerGroups.length ? peerGroups.map(g => `<div class="m3-card m3-card-pad" style="margin-bottom:8px">
        ...
        <div style="display:flex;gap:8px;margin-top:8px">
          <select class="m3-select pPeerSel" data-subj="${esc(g.subjectUserId)}" style="flex:1"><option value="">+ เพิ่มผู้ประเมิน</option>${personOptions('')}</select>
          <button type="button" class="m3-btn m3-btn--tonal pPeerAdd" data-subj="${esc(g.subjectUserId)}" style="width:auto;flex:none">${mi('add')}</button>
        </div></div>`).join('') : '<div class="m3-empty">—</div>');
  ...
  function selVal(cls, key, val) { const el = [...document.querySelectorAll('.' + cls)].find(x => x.dataset[key] === val); return el ? el.value : ''; }
  async function setBoss(sub) {
    const bossUserId = selVal('pBossSel', 'sub', sub);
    if (!bossUserId) { NT.toast('เลือกหัวหน้าก่อน', 'error'); return; }
    try { await NT.api('/360/pairing/setboss', { cycleId: pstate.cycleId, subordinateUserId: sub, bossUserId }); NT.toast('ตั้งหัวหน้าแล้ว'); await loadPairing(); paintPairing(); }
    catch (e) { NT.toast(e.message, 'error'); }
  }
  async function addPeer(subj) {
    const peerUserId = selVal('pPeerSel', 'subj', subj);
    if (!peerUserId) { NT.toast('เลือกผู้ประเมินก่อน', 'error'); return; }
    try { await NT.api('/360/pairing/addpeer', { cycleId: pstate.cycleId, subjectUserId: subj, peerUserId }); NT.toast('เพิ่มผู้ประเมินแล้ว'); await loadPairing(); paintPairing(); }
    catch (e) { NT.toast(e.message, 'error'); }
  }
