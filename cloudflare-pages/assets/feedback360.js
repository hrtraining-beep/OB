...
(function () {
  ...
  async function renderPairing(cycleId) {
    pstate.cycleId = cycleId; pstate.view = 'hub';
  ...
  const backToHub = () => { pstate.view = 'hub'; paintPairing(); };
  ...
  function paintSubjectReport() {
    ...
    NT.render(NT.m3Shell('profile', `
      ...
      <section class="m3-section"><h3 class="m3-section-label">สรุปสำหรับพนักงาน (HR แก้ไขได้)</h3>
        <p class="m3-save-hint" style="text-align:left;margin:0 0 6px">ระบบร่างให้จากคะแนน · ปรับถ้อยคำก่อนเก็บ (เน้นคำ + แนวทางพัฒนา)</p>
        ...
        <button type="button" class="m3-btn" id="rSave" style="margin-top:8px">${mi('save')}บันทึกสรุป</button>
      </section>
    ...
    document.getElementById('rSave').addEventListener('click', saveSubjectReport);
  }
  async function saveSubjectReport() {
    const d = srstate.data;
    ...
    try {
      await NT.api('/360/report/save', { cycleId: srstate.cycleId, subjectId: srstate.subjectId, finalSummary, draftSummary: d.draftSummary || '' });
      NT.toast('บันทึกสรุปแล้ว');
    } catch (e) { NT.toast(e.message, 'error'); } finally { restore(); }
  }
  ...
  window.FB360 = { renderHome, renderMyFeedback };
})();
